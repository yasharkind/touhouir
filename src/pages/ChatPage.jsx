...
const backendws = "ws://localhost:3000/ws"
const backendupload = "http://localhost:3000/upload"

const classifyMessage = (msg) => {
    const imgPattern = /^http:\/\/[^\/]+\/files\/.+(\.(jpg|jpeg|png|webp))$/;
...
const ChatPage = () => {
    ...
    return <>
        <audio
            ...
            preload="auto"
        />
        <div className='flex flex-col h-dvh scheme-home'  onKeyUp={k => {if (k.key == "Shift"){shift=false}}} onKeyDown={k => {if (k.key == "Shift"){shift=true}}}>``
            <div className="w-full h-9/10 max-h-9/10 overflow-auto flex flex-col-reverse p-4" >
                {_messages.toReversed().map((message, index) => {
                    ...
                    var border2 = "color-7 mb-2 mt-2 text-xl"
                    ...
                    switch (classifyMessage(message.content)) {
                        case "image":
                            return <div className={`${border} dejavu mt-2`}>
                                ...
                                <img src={message.content} className='max-w-1/2' /></div>
                        case "audio":
                            return <div className={`${border} dejavu mt-2`}>
                                <div className={border2}>
                                ...
                                <audio src={message.content} className='max-w-1/2' controls/></div>
                        case "video":
                            return <div className={`${border} dejavu mt-2`}>
                                <div className={border2}>
                                ...
                                <video src={message.content} className='max-w-1/2' controls/></div>
                    }
                    return <div className={`color-4 dejavu ${border}`} dir="auto">
                        <div className="flex flex-row">{border && 
                            <div className='color-7 mb-2 mt-2'>
                        {message.sender}:
                        </div>}</div><div className='color-7 inline ml-2 text-xs'>{message.time}</div>
                        {message.content.split("\n").map(cnt => {
                            return <div className="ml-2 inline text-l">{cnt}</div>
                        })
                        }
                    </div>
                })}
                ...
                <div className="flex flex-row m-auto">
                    ...
                    <div className='w-full flex flex-row items-center '>
                        <div className="w-full flex flex-col">
                            <textarea ref={messageRef} placeholder='message' type="text" className='w-full color-3 bg-1 dejavu max-h-[50px] rounded-xl' onKeyDown={k => { if (k.key == "Enter" && !shift) SendToWs() }} />
